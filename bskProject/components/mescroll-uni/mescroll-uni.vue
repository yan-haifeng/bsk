...
<script>
	...
	export default {
		...
		computed: {
			...
			numTop() {
				return uni.upx2px(Number(this.top || 0))+20
			},
			fixedTop() {
			...
			let diyOption = {
				...
				down: {
					inOffset(mescroll) {
						...
						vm.downText = mescroll.optDown.textInOffset; // 设置文本	<!-- ------------------------------------有改动--------------------------------- -->
					},
					outOffset(mescroll) {
						// 下拉的距离大于offset那一刻的回调
						vm.scrollAble = false; // 禁止下拉,避免抖动 (自定义mescroll组件时,此行不可删)
						vm.isDownReset = false; // 不重置高度 (自定义mescroll组件时,此行不可删)
						vm.isDownLoading = false; // 不显示加载中
						vm.downText = mescroll.optDown.textOutOffset; // 设置文本	<!-- ------------------------------------有改动--------------------------------- -->
					},
					onMoving(mescroll, rate, downHight) {
						// 下拉过程中的回调,滑动过程一直在执行; rate下拉区域当前高度与指定距离的比值(inOffset: rate<1; outOffset: rate>=1); downHight当前下拉区域的高度
						vm.downHight = downHight; // 设置下拉区域的高度 (自定义mescroll组件时,此行不可删)
						vm.downRotate = 'rotate(' + 360 * rate + 'deg)'; // 设置旋转角度	<!-- ------------------------------------有改动--------------------------------- -->
					},
					showLoading(mescroll, downHight) {
						...
						vm.downHight = downHight; // 设置下拉区域的高度 (自定义mescroll组件时,此行不可删)
						vm.downText = mescroll.optDown.textLoading; // 设置文本	<!-- ------------------------------------有改动--------------------------------- -->
					},
